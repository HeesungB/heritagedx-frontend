...
│       ├── src/
│       │   ├── app/                 # Next.js App Router ((dashboard)/error.tsx, loading: clubs, trade-memos, trade-records, kpi). clubs 목록(`/clubs` — V2 모노톤 시안 적용, breadcrumb 헤딩 + ClubBrowserPanel 안 검색바·지역/가나다 필터·즐겨찾기 strip·전체 골프장 grid) + 상세(`/clubs/[code]` — V2 분할 카드: `ClubDetailHeader` + `ClubDetailTabs`(기본 정보/회원권) + 활성 탭에 따른 `ClubInfoForm` 또는 `ClubMembershipPanel` + sticky `ClubDetailActionBar`. `useClubRepository().getOne(code)` 으로 club·memberships 일괄 로드, `useAdminRepositories()` 의 clubs/memberships update/create/delete, `pickClubUpdatePayload` 로 PUT 페이로드 화이트리스트, `useFavoriteClubs` 별 토글). customers: 목록(`/customers` — V2 시안 panel + 9컬럼 grid table, URL state `?q&page&owner`) + 상세(`/customers/[id]` — crumb·페이저·인물 카드 + 기본 정보·상담 이력·거래 건수·보유 회원권·메모 5개 카드). kpi(`/kpi` — V2 통계 시안: `KpiPageBar`(고정 헤더 + 필터 mini select + 직접선택 popover) → `KpiRangeChips`(9 프리셋) → `KpiSummaryStrip`(3 KPI 셀) → 2-col grid(`KpiTrendPanel` recharts ComposedChart + `KpiEmployeePanel` list/progress). 차트/직원 패널 metric은 독립 토글. `useKpiSummary/useKpiSeries/useKpiByEmployee` 그대로 사용, `getDateRange`로 범위 표시)
│       │   ├── components/
│       │   │   ├── clubs/           # 골프장 검색·등록·상세 V2 시안 — ClubsPageHeading(breadcrumb + title + subtitle), ClubBrowserPanel(흰 패널 shell), ClubSearchBar(검색 input + ESC pill + "새 골프장 등록" 버튼, `showRegisterButton` 옵션으로 등록 페이지에서는 숨김), ClubFilterRows(지역 pill 7+전체 / 가나다 chip 14+0-9, disabled/active 상태), FavoriteRecentStrip(`useTopClubs(clubs, 8)` 통합 — 즐겨찾기+최근 카드 horizontal scroll + 좌우 scroll 버튼, 0건이면 섹션 숨김), ClubGrid(2/3/4컬럼 반응형 + 결과 카운트 + 필터 초기화), ClubCardV2(운영형태 미니 pill(회원제 보라/퍼블릭 초록) + 홀수 mono pill + 별 토글 + 이름 + 지역, 별·카드 클릭 propagation 분리), ClubEmptyState(loading/no-results/no-clubs), ClubQuickList(라벨 + 카운트 + 가로 스크롤 텍스트 칩 — 등록 페이지에서는 제거됐고 다른 곳에서 재활용 대기 중인 공용 컴포넌트), ClubRegisterPageHeading(상단 ChevronLeft + `골프장 목록` back link + 22px title + 필수 안내 subtitle), ClubRegisterFormCard(번호 카드 셸 — 01/02 mono 번호 + 14px title), ClubRegisterActionBar(sticky bottom, 좌측 안내문구 + 취소/등록 버튼, `type="submit"` 등록), ClubRegisterForm(`clubBaseSchema` zodResolver — 기본정보 6필드(코드/명 필수, 주소·홈페이지 wide) + 코스정보 4필드, 일자 input 우측 calendar 오버레이), ClubDetailHeader(breadcrumb `골프장 / {name}` + 24px title + 즐겨찾기 별 토글(`useFavoriteClubs`) + 메타 행: 지역·운영형태·홀), ClubDetailTabs(`기본 정보` / `회원권` 세그먼트 탭 — Building2/CreditCard 아이콘 + 하단 underline), ClubDetailActionBar(sticky bottom — 좌측 정보 노트 + 목록으로(Link) + 저장(form="club-info-form" requestSubmit), 회원권 탭에서는 노트 메시지 차분히 변경), ClubInfoForm(`clubDetailSchema` zodResolver — 6 카드(01 기본·02 소개·03 코스·04 연락처(read-only/empty-state)·05 비용·06 명의개서 비용), `ClubRegisterFormCard` 재사용 + 내부 Field/Input/Textarea/InputWithSuffix primitives, `CLUB_INFO_FORM_ID` 로 외부 액션 바에서 submit), ClubMembershipFormV2(`membershipSchema` zodResolver — 시안 `.mem-section` 8 분할: 기본정보/그린피/시세/분양·입회/예약 안내/회원 혜택/명의개서 담당자/표시 설정, 그린피 동적 row + DEFAULT_FEE_TYPES datalist, EXCLUDE_FROM_PUT 화이트리스트로 OS 미노출 필드 보존, submit 시 weekday/weekend Record 빌드), ClubMembershipPanel(회원권 탭 오케스트레이션 — 헤더(타이틀·갯수칩·추가) + 개인/법인 세그먼트 + 이름 칩 quick-select + 아코디언 list, 추가/수정 시 자동 펼침 토글 닫기, 빈 상태 영역, 삭제는 `onRequestDelete` 콜백으로 parent ConfirmModal 호출). 기존 `components/forms/MembershipForm.tsx` 는 V2 폼이 책임 인계받아 삭제됨.
...
│       │   │   ├── trade-records/   # 거래 내역 V2 시안 적용: TradeRecordsPageHeader(화살표 아이콘·총계/매수/매도/진행중), TradeRecordsSplitLayout(gap 없는 border-right 분할), TradeRecordsFilterBar(flat 스타일·workflow 칩·건수 표시·골프장/날짜 필터), TradeRecordList(3-col grid 행·subtle 선택 상태·잔금 indicator·그룹 헤더), TradeRecordDetailPanel(브레드크럼·고객명 타이틀·primary 요약카드·done/pending 타임라인·stat-tag 세금), TradeRecordFormModal(중앙 등록/수정 섹션형 모달)
│       │   │   ├── forms/           # ClubForm, DocumentForm, ScenarioForm (MembershipForm 은 `components/clubs/ClubMembershipFormV2` 로 이관)
│       │   │   ├── home/            # HelloHeader, KpiCard, RecentTradesList, RecentConsultationsList, ProfitBreakdown, MonthlyTrendPanel
│       │   │   ├── kpi/             # 통계 V2 시안 — KpiPageBar(타이틀·아이콘박스·날짜 mono 메타 + 직접선택/날짜기준/직원 mini select), KpiDateRangePopover(시작·종료 input + 검증, ESC/바깥클릭 취소, Enter 적용), KpiRangeChips(`PRESET_GROUPS` 9 칩 + custom 모드 시 비활성), KpiSummaryStrip(3-cell 노란색 톤 KPI: 거래 #CA8A04 / 순이익 #854D0E / 상담 #EAB308), KpiViewTabs(종합/거래/상담/순이익 segment — Trend/Employee 패널 공용), KpiTrendPanel(panel-head + recharts ComposedChart + 점선구분선 위 범례 deep/light/line), KpiTrendChart(recharts 재스타일 — 노란 3톤 + dashed grid + JetBrains Mono axis), KpiEmployeePanel(종합 모드 5컬럼 list / 단일 metric 모드 progress track + 0~max 축 footer, sum 또는 metric 내림차순 정렬). KpiFilterBar·KpiEmployeeComparison V1 컴포넌트 삭제됨.
│       │   │   ├── layout/          # Header (모노톤 V2 리디자인), PageContainer
...
│   │   ├── trade-records.ts  # 거래내역 표시 helper: workflow 라벨/톤, 액션 가능 여부, 금액 포맷, 계약일 그룹핑, count 집계
│   │   └── tax/              # 세금·수수료 도메인
│   │       ├── index.ts      # 배럴 export
...
└── (dashboard)/                          # 대시보드 그룹
    ...
    ├── page.tsx                          # 대시보드 홈
    ├── clubs/
    │   ├── page.tsx                      # 골프장 목록 — V2 모노톤 시안 적용. breadcrumb 헤딩 + `ClubBrowserPanel`(검색바 ESC pill·"새 골프장 등록" → `/clubs/new` / `ClubFilterRows`(지역 pill 7값+전체, 가나다 chip 14자음+0-9, available Set 으로 disabled 상태) / `FavoriteRecentStrip`(`useTopClubs(clubs, 8)` — 즐겨찾기+최근 통합, 0건 숨김, 184px 카드 horizontal scroll) / `ClubGrid`(2→3→4컬럼 반응형, 결과 카운트 + 필터 초기화 ghost 버튼, 무한 스크롤 — 페이지네이션 미적용)). 데이터/필터 로직(useData clubs, filterMode initial/region/null, search 시 필터 reset)은 기존 그대로 유지. 카드 클릭 시 `trackSelection` + `router.push('/clubs/<code>')`, 별 클릭 시 propagation stop + `toggleFavorite(code, {name, region, holes})`
    │   ├── new/page.tsx                  # 골프장 등록 — V2 모노톤 시안 적용. `ClubRegisterPageHeading`(상단 ← 골프장 목록 back link + 22px title + 필수 안내) → `ClubRegisterForm`(필수 code/name + 8 옵션 필드, sticky `ClubRegisterActionBar`). 상단 빠른 검색/필터/즐겨찾기 패널은 제거됨 — 검색은 `/clubs` 목록 페이지에서만. submit 은 `useAdminRepositories().clubs.create()` 직호출(POC alert fallback 유지).
    │   └── [code]/
...
**레이아웃** (`layout/`): `Header`, `PageContainer`
