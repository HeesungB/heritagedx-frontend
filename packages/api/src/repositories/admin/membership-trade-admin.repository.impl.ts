...
import type { IMembershipTradeAdminRepository } from "../../interfaces/admin/membership-trade-admin.repository";
import type { TradeListParams } from "../../types";

export class MembershipTradeAdminRepository
  implements IMembershipTradeAdminRepository
{
  ...
  async getAll(
    ...
    const endpoint = `/admin/membership-trades${queryString ? `?${queryString}` : ""}`;
    return this.api.get<MembershipTradesResponse>(endpoint);
  }

  async getById(id: string): Promise<ApiResponse<MembershipTrade>> {
    return this.api.get<MembershipTrade>(`/admin/membership-trades/${id}`);
  }

  async create(data: MembershipTradeInput): Promise<ApiResponse<MembershipTrade>> {
    return this.api.post<MembershipTrade>("/admin/membership-trades", data);
  }
  ...
  async update(
    ...
    return this.api.put<MembershipTrade>(`/admin/membership-trades/${id}`, data);
  }

  async delete(id: string): Promise<ApiResponse<void>> {
    return this.api.delete(`/admin/membership-trades/${id}`);
  }
