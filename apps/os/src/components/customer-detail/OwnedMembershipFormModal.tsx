...
export function OwnedMembershipFormModal({
  ...
  const initialClubCode = useMemo(() => {
    ...
    // initialValue.clubId 가 UUID 인 경우(대부분), code 와 다름. clubName 으로 매칭 시도.
  ...
  // 골프장 변경 시 회원권 초기화 (edit 모드에서 같은 골프장 유지하면 그대로 둠).
  useEffect(() => {
  ...
  // 회원권 옵션 dedup (이름 기준, 첫 매칭 id 대표 — ConsultationFormSections 와 동일 패턴)
